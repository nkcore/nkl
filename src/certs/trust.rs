...
fn trust_ca_macos(ca_cert_path: &Path) -> anyhow::Result<TrustResult> {
    ...
    if stderr.contains("authorization") || stderr.contains("permission") {
        Ok(TrustResult::PermissionDenied(format!(
            "permission denied. Try: sudo nkl trust"
        )))
    } else {
...
fn is_ca_trusted_linux(state_dir: &Path) -> bool {
    ...
    let debian_path = PathBuf::from("/usr/local/share/ca-certificates/nkl-ca.crt");
    if debian_path.exists() {
        if let Ok(installed) = fs::read(&debian_path) {
            if installed == our_pem {
                return true;
            }
        }
    }

    let rhel_path = PathBuf::from("/etc/pki/ca-trust/source/anchors/nkl-ca.pem");
    if rhel_path.exists() {
        if let Ok(installed) = fs::read(&rhel_path) {
            if installed == our_pem {
                return true;
            }
        }
...
fn trust_ca_debian(ca_cert_path: &Path) -> anyhow::Result<TrustResult> {
    let dest = PathBuf::from("/usr/local/share/ca-certificates/nkl-ca.crt");
    match fs::copy(ca_cert_path, &dest) {
        Ok(_) => {}
        Err(e) if e.kind() == std::io::ErrorKind::PermissionDenied => {
            return Ok(TrustResult::PermissionDenied(
                "permission denied. Try: sudo nkl trust".to_string(),
            ));
        }
        Err(e) => {
    ...
    match output {
        ...
        Err(e) if e.kind() == std::io::ErrorKind::PermissionDenied => Ok(
            TrustResult::PermissionDenied("permission denied. Try: sudo nkl trust".to_string()),
        ),
...
fn trust_ca_rhel(ca_cert_path: &Path) -> anyhow::Result<TrustResult> {
    let dest = PathBuf::from("/etc/pki/ca-trust/source/anchors/nkl-ca.pem");
    match fs::copy(ca_cert_path, &dest) {
        Ok(_) => {}
        Err(e) if e.kind() == std::io::ErrorKind::PermissionDenied => {
            return Ok(TrustResult::PermissionDenied(
...
fn trust_ca_arch(ca_cert_path: &Path) -> anyhow::Result<TrustResult> {
    let dest = PathBuf::from("/etc/ca-certificates/trust-source/anchors/nkl-ca.pem");
    match fs::copy(ca_cert_path, &dest) {
        Ok(_) => {}
        Err(e) if e.kind() == std::io::ErrorKind::PermissionDenied => {
            return Ok(TrustResult::PermissionDenied(
