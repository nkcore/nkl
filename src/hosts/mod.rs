...
const MARKER_START: &str = "# nkl-start";
const MARKER_END: &str = "# nkl-end";

/// Extract lines between the nkl markers (excluding the markers themselves).
...
pub fn extract_managed_block(content: &str) -> Option<Vec<String>> {
    ...
    if inside { None } else { None }
}

/// Remove the nkl-managed marker block from hosts file content.
...
/// Remove the nkl-managed block from the hosts file.
...
fn write_hosts_atomic(hosts_path: &Path, content: &str) -> io::Result<()> {
    ...
    let tmp_path = parent.join(".nkl-hosts.tmp");
