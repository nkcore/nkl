...
///
/// Running nkl through npx/pnpm dlx is unreliable because the daemon
...
pub fn check_npx_execution() {
    if is_npx_or_dlx() {
        eprintln!("Error: nkl should not be run via npx or pnpm dlx.");
        eprintln!("Install globally instead:");
        eprintln!("  npm install -g nkl");
        std::process::exit(1);
    }
}
