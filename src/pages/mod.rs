...
<title>{status} {status_text} - NKL</title>
<style>
:root {{
  --bg: #ffffff;
  --fg: #111111;
...
<div class="footer">
  Powered by <a href="https://github.com/nkcore/nkl">NKL</a>
</div>
</body>
</html>"#,
        status = status,
    ...
    let routes_html = if routes.is_empty() {
        ...
        for r in routes {
            let prefix_label = if r.path_prefix == "/" {
                ...
                format!("{}", escape_html(&r.path_prefix))
            };
...
<div style="margin-top:24px">
  ...
  <div class="card">
    ...
    <pre><code>nkl run -- your-start-command</code></pre>
  </div>
</div>"#,
        host = safe_host,
        port = port,
...
pub fn render_loop_detected_body() -> String {
    r#"<div class="tip-box">
  <p>This request has passed through NKL too many times, indicating a proxy loop.</p>
...
<div style="margin-top:24px">
  ...
  <div class="card">
    ...
    <pre><code>nkl alias myapp.localhost 3000 --change-origin</code></pre>
    <p style="margin-top:8px">Or when using <code>nkl run</code>:</p>
    <pre><code>nkl run --change-origin -- npm start</code></pre>
  </div>
</div>"#
        .to_string()
}
