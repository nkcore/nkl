...
use super::{NKL_HEADER, RouteCache};
    ...
    let proxy_handle = tokio::spawn(async move {
        if let Ok((stream, _)) = proxy_listener.accept().await {
            ...
            let service = service_fn(move |req: Request<hyper::body::Incoming>| {
                ...
                async move { handle_request(req, proxy_port, 10, cache, false).await }
            });
...
async fn test_http2_prior_knowledge_routing() {
    ...
    match resp {
        Ok(r) => {
            ...
            assert!(
                r.headers().contains_key(NKL_HEADER),
                "response should contain X-NKL header"
            );
        }
        Err(e) => {
