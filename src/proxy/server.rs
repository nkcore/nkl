...
pub async fn start_proxy(config: &Config) -> anyhow::Result<()> {
    ...
    tracing::info!("proxy listening on {}://localhost:{}", proto, port);
                    ...
                    Ok((stream, _)) => {
                        ...
                        tokio::task::spawn(async move {
                            if let Some(acceptor) = acceptor {
                                handle_connection_with_tls(stream, acceptor, port, max_hops, cache).await;
                            } else {
                                let io = TokioIo::new(stream);
                                serve_http(io, port, max_hops, cache, false).await;
                            }
                        });
                    }
                    Err(err) => {
    ...
    if peek_buf[0] == 0x16 {
        ...
        match acceptor.accept(stream).await {
            Ok(tls_stream) => {
                ...
                serve_http(io, port, max_hops, route_cache, true).await;
            }
            Err(e) => {
        ...
        let io = TokioIo::new(stream);
        serve_http(io, port, max_hops, route_cache, false).await;
    }
}

/// Serve HTTP/1.1 or HTTP/2 on an already-accepted I/O stream.
async fn serve_http<I>(io: I, port: u16, max_hops: u32, route_cache: RouteCache, is_tls: bool)
where
    I: hyper::rt::Read + hyper::rt::Write + Unpin + Send + 'static,
{
    let service = service_fn(move |req: Request<Incoming>| {
        let cache = Arc::clone(&route_cache);
        async move { handle_request(req, port, max_hops, cache, is_tls).await }
    });
