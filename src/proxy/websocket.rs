...
use super::{NKL_HEADER, NKL_HOPS_HEADER, response, strip_path_prefix};
...
pub(super) async fn handle_upgrade(
    ...
    if status_code != 101 {
        ...
        builder = builder.header(NKL_HEADER, "1");
        ...
        return Ok(builder.body(Full::new(Bytes::from(body_data))).unwrap());
    }
    ...
    tokio::task::spawn(async move {
        ...
        if !extra_data.is_empty() {
            if let Err(e) = tokio::io::AsyncWriteExt::write_all(&mut client_io, &extra_data).await {
