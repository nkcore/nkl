...
impl RouteStore {
    ...
    pub fn ensure_dir(&self) -> Result<(), NKLError> {
        if !self.dir.exists() {
    ...
    pub fn load_routes(&self) -> Result<Vec<RouteMapping>, NKLError> {
    ...
    pub fn add_route(
        ...
        if let Some(existing) = routes.iter().find(|r| {
            ...
            if existing.pid != pid
                ...
                && !force
            {
                return Err(NKLError::RouteConflict {
    ...
    pub fn remove_route(&self, hostname: &str, path_prefix: Option<&str>) -> Result<(), NKLError> {
        self.ensure_dir()?;
        let _lock = self.acquire_lock()?;
    ...
    fn save_routes(&self, routes: &[RouteMapping]) -> Result<(), NKLError> {
    ...
    fn acquire_lock(&self) -> Result<LockGuard, NKLError> {
        for _ in 0..LOCK_MAX_RETRIES {
            match fs::create_dir(&self.lock_path) {
                ...
                Err(e) => return Err(NKLError::Io(e)),
            }
        }
        Err(NKLError::LockFailed)
    }
}
...
mod tests {
    ...
    fn test_route_conflict() {
        ...
        // pid 0 means "static alias", always considered alive
        store
            .add_route("myapp.localhost", 4000, 0, false, false, "/", false)
            .unwrap();
