...
pub enum RouteKind {
    /// Static alias (pid=0), registered via `nkl alias`
    Alias,
...
fn discover_active_state_dir() -> (PathBuf, u16) {
    ...
    if let Some(port) = read_proxy_port(&user_dir) {
        if let Some(pid) = read_proxy_pid(&user_dir) {
            if is_alive(pid) {
    ...
    if let Some(port) = read_proxy_port(&sys_dir) {
        if let Some(pid) = read_proxy_pid(&sys_dir) {
            if is_alive(pid) {
                return (sys_dir, port);
            }
        }
...
pub fn get_proxy_status() -> ProxyStatus {
    ...
    let running = pid.map_or(false, |p| is_alive(p));
...
pub fn get_route_statuses(state_dir: &Path, proxy_port: u16, tls: bool) -> Vec<RouteStatus> {
    ...
    routes
        ...
        .map(|r| {
            ...
            let url = format_url(&r.hostname, proxy_port, tls);
...
pub fn print_status() {
    ...
    // -- Routes section --
    let routes = get_route_statuses(&proxy.state_dir, proxy.port, proxy.tls);
    println!();
    ...
    for route in &routes {
        ...
        let kind_label = match route.kind {
            RouteKind::Alias => "alias".to_string(),
            RouteKind::Dynamic => {
...
mod tests {
    ...
    #[test]
    fn test_route_kind() {
        assert_eq!(RouteKind::Alias, RouteKind::Alias);
        assert_ne!(RouteKind::Alias, RouteKind::Dynamic);
    }
    ...
    fn test_get_route_statuses_with_routes() {
        ...
        // pid 0 = alias, always alive
        assert_eq!(statuses[0].kind, RouteKind::Alias);
